...
There are multiple options for the values given to $y_0$ and $y_{N-1}$. We chose to set them at $0$. Once the values were set up, the system was solved. The whole process is summed up by algorithm \ref{interp_precalc}.
...
The result was right, but one problem remained : the $interp(x)$ evaluation complexity was $\mathcal{O}(N^3)$ ($N$ being the number of points), because the system of equation had to be solved at each evaluation.
...
To sum up, all that had to be done was to compute the formal derivative of $A$, and all the other coefficient were easily calculated by using it. This algorithm complexity was exactly the same as the previous one:  $\mathcal{O}(N^3)$ to obtain the interpolating function, and  $\mathcal{O}(N)$ to evaluate it.
...
As can be seen in figure \ref{error_integration} and even more in figure \ref{log_error_integration} the Simpson method converges faster than the other ones.
...
On these graphs, the function chosen was representative of the global results found, but for the function $f(x) = \sqrt{1-x^2}$ on the interval $[-1,1]$ all the methods converge with the same speed. We think that it's linked to the fact that this function is not derivable on $-1$ and $1$, but we did not have the time to develop it.
